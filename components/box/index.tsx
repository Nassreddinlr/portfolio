...
import { ClientThemeSwitcher } from "../client-theme-switcher"

export function WaitlistWrapper({ children }: PropsWithChildren) {
  return (
    <div
      className={clsx(
        "w-full mx-auto max-w-[600px] flex flex-col justify-center items-center bg-gray-1/85 pb-0 overflow-hidden rounded-2xl",
    ...
    >
      ...
      <footer className="flex justify-between items-center w-full self-stretch px-8 py-3 text-sm bg-gray-12/[.07] overflow-hidden">
        <p className="text-xs text-slate-10">© 2025 Nassreddine Larbi Aissa. All rights reserved.</p>
        <ClientThemeSwitcher />
      </footer>
    </div>
  )
